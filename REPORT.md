...
Do **not** set `CUA_HITL_AUTO_RESUME` for a headed product demo. Tests may auto-resume takeover HITL **after** a test-only helper clicks OK on the same Playwright page; that auto path does not log `actor=human` and does not approve policy-required actions.
